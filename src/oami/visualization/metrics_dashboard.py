...
class MetricsDashboard:
    ...
    def __init__(self, title: str = "Trading Performance Dashboard", figsize: tuple[int, int] = (14, 8)) -> None:
        self.title = title
    ...
    def update(self, equity: float, action_dict: Dict[str, float], timestamp: Optional[pd.Timestamp] = None) -> None:
        ...
        self.timestamps.append(timestamp if timestamp is not None else pd.Timestamp.utcnow())
    ...
    def compute_metrics(self) -> Dict[str, float]:
        ...
        win_rate = len(positive_returns) / len(returns[returns != 0]) if len(returns[returns != 0]) > 0 else 0.0
    ...
    def plot_equity_curve(self, ax: Optional[plt.Axes] = None) -> plt.Axes:
        """Plot the account value curve with drawdown shading."""
        ...
        drawdown_series, _ = self._compute_drawdown_curve(pd.Series(self.account_values))
        ax.fill_between(
            times,
    ...
    def plot_action_heatmap(self, ax: Optional[plt.Axes] = None) -> Optional[plt.Axes]:
        """Heatmap of action intensities per ticker."""
        ...
        if ax is None:
            _, ax = plt.subplots(figsize=self.figsize)
        sns.heatmap(matrix, cmap="coolwarm", ax=ax, cbar=True, center=0.0, yticklabels=tickers)
    ...
    def show_summary_table(self, ax: plt.Axes) -> None:
        """Display the metrics dictionary as a table."""
    ...
    def render_dashboard(self) -> None:
        """Render all plots in a combined dashboard and save to disk."""
        ...
        if heatmap_ax is None:
            axes[1, 1].axis("off")
            axes[1, 1].text(0.5, 0.5, "Action data unavailable", ha="center", va="center")

        self.compute_metrics()
        self.show_summary_table(ax=axes[2, 1])

        returns_series = pd.Series(self.returns)
        rolling_sharpe = returns_series.rolling(window=50).mean() / (returns_series.rolling(window=50).std() + 1e-8)
        rolling_vol = returns_series.rolling(window=50).std()
        axis_index = self.timestamps if self.timestamps else range(len(rolling_sharpe))
        axes[2, 0].plot(axis_index, rolling_sharpe, label="Rolling Sharpe", color="tab:purple")
        axes[2, 0].plot(axis_index, rolling_vol, label="Rolling Vol", color="tab:orange")
