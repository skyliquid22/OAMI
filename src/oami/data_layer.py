...
def _download_stocks_flatfile(
    ...
    for year, month in _progress_iter(
        ...
        if flatfile_client is None:
            try:
                flatfile_client = PolygonFlatfileClient()
        ...
        except RuntimeError as exc:  # pragma: no cover - network call
            LOGGER.warning(
...
def _subtract_years(anchor: date, years: int) -> date:
    ...
    try:
        ...
        # Handle leap-day wrap-around by stepping back to Feb 28.
...
def get_option_flatfile_data(
    ...
    file_start = pd.to_datetime(as_of).date()
    file_end = pd.to_datetime(as_to).date() if as_to else file_start
    ...
    if missing_dates:
        if flatfile_client is None:
            try:
                ...
                logging.warning("Unable to initialize PolygonFlatfileClient", extra={"error": str(exc)})
                missing_dates = []
        if missing_dates:
            for expiration in _progress_iter(
                missing_dates,
                ...
                filename = path.name
                month_key = (expiration.year, expiration.month)
                if month_key not in remote_listing_cache:
                    try:
                        ...
                        continue
                if filename not in remote_listing_cache[month_key]:
                    logging.info(
                ...
                try:
                    flatfile_client.download_flatfile(
                        ...
                        expiration.month,
                        day=expiration.day,
                        available_files=remote_listing_cache.get(month_key),
                    )
                except RuntimeError as exc:
                    logging.warning(
...
def get_stock_flatfile_data(
    ...
    ticker : str
        Target stock symbol.
    start : str, default ``"2025-01-01"``
        Inclusive start of the date window.
    end : str | None, default ``None``
        Inclusive end of the window. When omitted the current date is used.
    """
